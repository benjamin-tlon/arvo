...
++  type-to-type-xray
  ...
  ++  enter
    ...
    ^-  type-xray
    ~&  'enter'
    |^  =+  [x=xray st=entry-table]=entry
        [x (build-loop-map table.st)]
        +>+>+(xray xray, loop-map (build-loop-map table.st))
  ::
  ...
  ++  entry
    ...
    ?:  (~(has in trace) ty)
      :-  `xray`count.state
      =/  stub=[type xray]  [ty [count.st *zray]]
      st(count +(count.st), table (~(put by table.st) stub))
    =^  xray  state  main(trace (~(put in trace) type))
    ?@  xray  [xray state]
    =/  new  (~(get by table.state) type)
    ?~  new  [xray state]
    =*  idx  index.u.new
    =.  entry-unit.meta.xray  `idx
    ?<  (~(has by table.state) idx)
    =.  table.st  (~(put by table.state) type [p.u.new xray])
    [xray st]
  ::
  ...
  ++  main
    ...
    ?-  ty
      ...
      [%cell *]  =^  hed  st  $(ty p.ty)
                 ...
                 [[ty wray] st]
      [%core *]  =^  wray  st  (core p.ty q.ty)
                 [[ty wray] st]
      [%face *]  =^  xray  st  $(ty q.ty)
                 =*  wray  [*meta %face p.ty xray]
                 [[ty wray] st]
      [%hint *]  =^  wray  st  (hint p.ty q.ty)
                 [[ty wray] st]
      [%fork *]  =^  wray  st  (fork p.ty)
                 [[ty wray] st]
  ...
  ++  core
    |=  [[tr=trace ty=type] st=state]
    ^-  [wray state]
    =^  payload-xray  state  (main [[tr payload-type main(type payload-type)
  ...
  ++  measure
    ...
    ++  analyze
      ...
      ?^  shape-unit.meta.xray  ~&  %xray-already-measured
                                +>
      ::
      =^  new-recipe-set  loop-map
        ?~  recipe-set.meta.xray  [recipe-set.meta.xray loop-map]
        ^-  [(set recipe) _loop-map]
        =/  recipes=(list recipe)
          ~(tap in ^-((set recipe) recipe-set.meta.xray))
        =^  finished-items=(list recipe)  loop-map
          =*  trav-recipes  (traverse-left recipe recipe _loop-map)
          =*  trav-xrays    (traverse-left ^xray ^xray _loop-map)
          ...
          ?-    -.r
            ...
            [r(list new-xrays) st]
          ==
        [(~(gas in *(set recipe)) finished-items) loop-map]
      =.  recipe-set.meta.xray  new-recipe-set
      ...
      =.  loop-set  ?~  entry-unit.meta.xray  loop-set
                    ~&  [%added-to-loop-set u.entry-unit.meta.xray]
      ...
      ::
        ...
        =.  new-xray  new-xray(recipe-set.meta ~)
        +>+>+(xray new-xray)
      ==
    --
  ::
...
:: +hoon-printer =<  -  !>  *$%([%e *] [%n (list (list (list $-($@(%g [* ^]) $^([[b=@ *] *] [$?(%n ~ %b) *])))))] [%u *])
:: +hoon-printer =<  -  !>  *(list $-(%a %b))
